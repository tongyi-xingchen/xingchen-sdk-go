...
// APIClient manages communication with the XingChen 开放接口定义 API vv1
...
type APIClient struct {
	...
	CharacterApiSub *CharacterApiSubService
...
func (c *APIClient) prepareRequest(
	...
	if c.cfg.Version == V2 {
		localVarRequest.Header.Add(GATEWAY_HEADER_APPCODE, APP_CODE)
		serviceRouter := getServiceRouteHeader(localVarRequest.URL.Path, localVarRequest.Header)
		localVarRequest.Header.Add(GATEWAY_HEADER_SERVICE_ROUTER, serviceRouter)
	}
...
func getServiceRouteHeader(apiPath string, header http.Header) string {
	...
	for k, v := range V2_PATH_ROUTE_MAP {
		if strings.HasPrefix(apiPath, k) {
			return v
		}
	}
	return ""
